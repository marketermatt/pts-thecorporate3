...
add_action( 'tgmpa_register', 'thecorporate_register_required_plugins' );
function thecorporate_register_required_plugins() {
$plugins = array(
array(
'name'               => 'The Corporate Shortcodes',
'slug'               => 'thecorporate',
'source'             => 'thecorporate.zip',
'required'           => true,
'version'            => '1.0.0',
)
);
...
$config = array(
'default_path' => get_stylesheet_directory() . '/lib/plugin/',
'menu'         => 'thecorporate-install-plugins',
'has_notices'  => true,
'dismissable'  => true,
'dismiss_msg'  => '',
'is_automatic' => false,
...
'strings'      => array(
'page_title'                      => __( 'Install Required Plugins', 'thecorporate' ),
'menu_title'                      => __( 'Install Plugins', 'thecorporate' ),
'installing'                      => __( 'Installing Plugin: %s', 'thecorporate' ),
'oops'                            => __( 'Something went wrong with the plugin API.', 'thecorporate' ),
...
'return'                          => __( 'Return to Required Plugins Installer', 'thecorporate' ),
'plugin_activated'                => __( 'Plugin activated successfully.', 'thecorporate' ),
'complete'                        => __( 'All plugins installed and activated successfully. %s', 'thecorporate' ),
'nag_type'                        => 'updated'
)
);
...
'menu_title'                     => __( 'Install Plugins', 'thecorporate' ),
'installing'                     => __( 'Installing Plugin: %s', 'thecorporate' ),
'oops'                           => __( 'Something went wrong.', 'thecorporate' ),
...
'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins' ),
'return'                         => __( 'Return to Required Plugins Installer', 'thecorporate' ),
'dashboard'                      => __( 'Return to the dashboard', 'thecorporate' ),
'plugin_activated'               => __( 'Plugin activated successfully.', 'thecorporate' ),
'activated_successfully'         => __( 'The following plugin was activated successfully:', 'thecorporate' ),
'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'thecorporate' ),
'dismiss'                        => __( 'Dismiss this notice', 'thecorporate' ),
);
...
protected function do_plugin_install() {
    ...
    if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
        ...
        if ( empty( $complete ) ) {
            echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'thecorporate' ) . '">' . __( 'Return to the Dashboard', 'thecorporate' ) . '</a>' ) . '</p>';
...
public function notices() {
    ...
    if ( ! empty( $message ) ) {
        ...
        foreach ( $message as $type => $plugin_groups ) {
            ...
            $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'thecorporate' ), $imploded, $count ) . '</p>';
        }

        // Setup variables to determine if action links are needed.
        $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'thecorporate' ) . '</a>' : '';
        $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'thecorporate' ) . '</a>'  : '';
        ...
        if ( ! empty( $this->strings['nag_type'] ) ) {
            add_settings_error( 'tgmpa', 'thecorporate', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
        } else {
            add_settings_error( 'tgmpa', 'thecorporate', $rendered, $nag_class );
        }
    }
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'thecorporate' );
                        // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'thecorporate' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'thecorporate' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'thecorporate' ) : __( 'Recommended', 'thecorporate' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'thecorporate' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'thecorporate' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'thecorporate' ) . ' %2$s">' . __( 'Install', 'thecorporate' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'thecorporate' ) . ' %2$s">' . __( 'Activate', 'thecorporate' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'thecorporate' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'thecorporate' ),
                'source' => __( 'Source', 'thecorporate' ),
                'type'   => __( 'Type', 'thecorporate' ),
                'status' => __( 'Status', 'thecorporate' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'thecorporate' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'thecorporate' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'thecorporate' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'thecorporate' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>', 'thecorporate' ).'&#8230;';
                $this->strings['unpack_package']      = __( 'Unpacking the package', 'thecorporate' ).'&#8230;';
                $this->strings['installing_package']  = __( 'Installing the plugin', 'thecorporate' ).'&#8230;';
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'thecorporate' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'thecorporate' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'thecorporate' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'thecorporate' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'thecorporate' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'thecorporate' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'thecorporate' ) . '</span><span class="hidden">' . __( 'Hide Details', 'thecorporate' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'thecorporate' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'thecorporate' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'thecorporate' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'thecorporate' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'thecorporate' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'thecorporate' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'thecorporate' ) . '</span><span class="hidden">' . __( 'Hide Details', 'thecorporate' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'thecorporate' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'thecorporate' );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'thecorporate' ) . '">' . __( 'Return to the Dashboard', 'thecorporate' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
